...
use johnsnook\tui\helpers\Point;
use johnsnook\tui\helpers\Rectangle;
...
abstract class Boxy {
    ...
    /**
     *
     * @param \johnsnook\tui\helpers\Point $p
     * @param \johnsnook\tui\helpers\Rectangle $r
     * @return boolean
     */
     ...
     *
     * @param \johnsnook\tui\elements\Element $subject The element whose
    ...
    public static function applyStyle(Element $subject) {
        ...
        if ($subject->width === Dimensions::NOT_SET) {
            if (gettype($style->width) === 'integer' && $subject->width === Dimensions::NOT_SET) {
                ...
                $subject->width = floor($container->width * ($style->width / 100));
            }
        }
        if ($subject->height === Dimensions::NOT_SET) {
            if (gettype($style->height) === 'integer' && $subject->height === Dimensions::NOT_SET) {
                ...
                $subject->height = floor($container->height * ($style->height / 100));
            }
        }
         ...
         */
        $addY = ($style->position !== Style::RELATIVE ? $container->top : 0);
        $addX = ($style->position !== Style::RELATIVE ? $container->left : 0);

        if ($subject->left === Point::NOT_SET) {
            if ($style->pull & Style::LEFT) {
                $subject->left = $addX + $subject->style->marginLeft + $container->style->paddingLeft;
            } elseif ($style->pull & Style::RIGHT) {
     ...
     * }
     *
     * @param \johnsnook\tui\helpers\Rectangle $r1
     * @param \johnsnook\tui\helpers\Rectangle $r2
     */
